...
def main(_):
	...
	print("dataset list",dir_list)
	yaml_dir=None
	for i in dir_list:
		if(i.split('.')[-1] == r'yaml'):
			print("found yaml in",i)
			yaml_dir=os.path.join(dataset, i)
	#print("yaml check:",os.listdir(dataset).split('.')[-1])
	...
	if yaml_dir is not None: 
	    ...
	    examples_list = get_imgs_from_yaml(yaml_dir)
		...
		temp_path=data['path']
		temp_path_sp=temp_path.split(':')
		#print("first half",temp_path_sp[0])
		#print("second half",temp_path_sp[1])
		first_half=temp_path_sp[0][:-1]
		print("first half pp",first_half)
		
		second_half1=temp_path_sp[1].split('\\')
		print("secondhalf last",second_half1[-1])
		valid_path=first_half+second_half1[-1]
                data['path'] = create_jpg_imgs(valid_path)
